...
def getRoute():
    ...
    string = "https://maps.googleapis.com/maps/api/directions/json?origin=" + str(origin[0]) + "," + str(
        origin[1]) + "&destination=" + str(destin[0]) + "," + str(destin[
                                                                      1]) + "&key=AIzaSyDgerVrDyc-PTmUb2CUoJKO-QdeOGxVZls&avoid=indoor&units=km&transit_mode=bus&transit_routing_preference=less_walking&mode=transit&departure_time=now&alternatives=true"
...
def googleName(normalName):
    ...
    string = "https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyDgerVrDyc-PTmUb2CUoJKO-QdeOGxVZls&address=" + normalName
    response = requests.get(string)
    response = response.json()
